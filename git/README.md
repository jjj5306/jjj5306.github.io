...
- pull request는 협업에서 주로 사용하며, 내가 만든 브랜치에 대해 협업자에게 코드를 봐주기를 요청하고  
 머지 권한이 있는 사람에게 머지해주기를 요청한다. 특히 토픽 브랜치를 만들고 작업하다가, 마스터 브랜치에 추가할 때 많이 사용한다.

- pull request를 할 때 reviewers, assignees를 등록할 수 있고 각각 리뷰를 달아줬으면하는 사람과 작업할 사람을 등록한다.

- `draft pull request`를 생성할 수 있는데, 이는 머지는 하지않고 코드를 봐주기만을 원할 때 사용한다.

- pull request를 눌러보면 `file changes`라는 칸이 뜨는데, 이 칸에서는 이 브랜치에서 수정한 최종 워킹스페이스를 기존 브랜치와 비교하면서 볼 수 있다.  
 이 칸에서 라인별로 선택을 하면서 리뷰나 요청사항을 남길 수 있고 그 라인에 대해 바로 수정하여 커밋할 수 있는 기능들도 있다.

 - pull request는 협업 상황에서 충돌을 최소화하기 위한 중요한 기능이다.